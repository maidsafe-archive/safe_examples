...
app.post("/auth", jsonParser, (req, res) => {
  ...
  if (!body.app || !body.app.name || !body.app.id || !body.app.version || !body.app.vendor){
    return res.status(400).json({"error": "INCOMPLETE", "messages": "missing app information"})
  }
  if (!body.permissions || body.permissions.length == 0) {
    return res.status(400).json({"error": "INCOMPLETE", "messages": "missing permissions information"})
  }
  ...
  if (!body.permissions.indexOf("LOW_LEVEL_API") === -1) {
    return res.status(403).json({"error": "INCOMPLETE", "messages": "you need to ask for LOW_LEVEL_API permission for this mock"})
  }
...
app.post("/structured-data/:id", rawbody, (req, res) => {
  if (req.headers.authorization != 'Bearer ' + TOKEN) {
    return res.status(403).json({"error": "UNAUTHORIZED", "messages": "you are not allowed to run this method"})
  }
  const tagType = parseInt(req.headers['Tag-Type'] || 500, 10);
  if (tagType != 500 && tagType != 501 && tagType <= 15000) {
    return res.status(400).json({"error": "DISALLOWED_TAG_TYPE", "messages": "Header Tag-Type must be 500, 501 or above 15000"})
  }

  const encryption = (req.headers['Encryption'] || 'NONE').toUpperCase();
  if (![null, "NONE", "SYMMETRIC", "HYBRID"].indexOf(encryption)){
    return res.status(400).json({"error": "DISALLOWED_ENCRYPTION", "messages": "Header Encryption must be NONE, SYMMETRIC or HYBRID"})
  }
  if (!req.rawBody) {
    return res.status(400).json({"error": "EMPTY_CONTENT", "messages": "you need to provide some data"})
  }

  if (STORE[req.params.id]){
    res.status(409).json({"error": "CONFLICT", "message": "item already exists"})
  }
...
app.get("/structured-data/handle/:id", (req, res) => {
  ...
  if (!STORE[req.params.id]) {
    return res.status(404).json({"error": "NOT_FOUND", "message": "Structured Data not found."})
  }
